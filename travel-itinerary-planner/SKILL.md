...
- `references/scheduling-method.md` —— **排程方法:一天要按什麼順序搭起來**(錨點 → 用餐 → 地理範圍 → 項目 → 移動行 → 驗算)
...
4. **蒐集實際資訊**(航班/天氣/景點/營業時間/交通時間),依 `references/scheduling-method.md` 的順序排定行程。
...
- `weather_fetch`:確認天氣,安排室內外備案,以及行前清單的衣物建議(用法有陷阱,見下方)。
...
**排程有固定的順序,不要憑感覺排。完整方法在 `references/scheduling-method.md`,排每一天之前先讀。** 摘要:
