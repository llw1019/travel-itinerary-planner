...
視覺質感不能取代內容密度。完整的密度標準在 `references/output-checklist.md` 第三節,排完後逐項對照。最核心的四條:
...
必含 13 個區塊(製表資訊、行程總覽、一頁式速查總表、每日時間軸、航班、住宿、司機接送摘要、訂位狀態追蹤表、備案、費用估算、行前準備清單、實用資訊附錄、緊急頁),各區塊要有哪些欄位見規格檔。
