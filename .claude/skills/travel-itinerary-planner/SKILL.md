...
9. **交付時主動問,而且分兩題**:第一題是**對這一份的加工**(加圖片、做深度介紹、發布成 artifact),第二題是**另外一份文件**(B5 列印手冊、A4 精簡版)。問卷工具一題最多 4 個選項,塞不下六個。使用者不會知道有這些選項,除非你講。
...
**衍生版一律從定案的詳細版重新生成,不獨立編輯。** 使用者對衍生版提出的內容修改,改詳細版再重生成;只有版面調整才直接在衍生版上做。詳細版之後每改一次,存在的衍生版都要重生成(Step 8 同步鏈第 9 點)。
...
- **三個不能砍**:一頁式速查總表與硬約束清單(決策者最常看的一頁)、緊急頁壓成一行(砍掉會出事)、**各項目的介紹一字不減**(敘事是它存在的理由)
