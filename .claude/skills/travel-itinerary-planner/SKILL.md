...
8. **交付時主動問四件事**:要不要加使用者自己的圖片、要不要發布成 artifact、要不要做 B5 列印手冊、要不要做 A4 精簡版。使用者不會知道有這些選項,除非你講。
9. **衍生版是另一個階段**:B5 列印手冊與 A4 精簡版,只在使用者確認詳細版內容通過、並且**分別**同意之後才做。兩份要分開問,不要一次產兩份。
...
2. **先確認是哪一種修改。** 「換內容」(改地點、加項目、調時間)才需要重排;**「只加圖片」「只出衍生版」「只改排版」不必動行程**,直接跳 Step 8 / Step 9,不要藉機重排一遍
...
**只做詳細版,不要同時做任何衍生版。** 內容還在修改的階段維護多份,改一個時間、換一家餐廳都要改兩三個檔案,成本翻倍而且很容易版本不一致。B5 列印手冊與 A4 精簡版都是 Step 9,等內容定案再說。
...
**衍生版一律從定案的詳細版重新生成,不獨立編輯。** 使用者對衍生版提出的內容修改,改詳細版再重生成;只有版面調整才直接在衍生版上做。詳細版之後每改一次,存在的衍生版都要重生成(Step 7 同步鏈第 9 點)。
